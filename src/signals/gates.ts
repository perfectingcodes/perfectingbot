...
export async function runGates(
  fomo: FomoClient, trigger: TradeEvent, tokenPriceUsd: number,
): Promise<GateResult[]> {
...
async function honeypotGate(fomo: FomoClient, chain: Chain, token: string): Promise<GateResult> {
  try {
    ...
    if (!holder) return { name: "honeypot", passed: false, detail: "no holder available to simulate a transfer" };
    ...
    const ok = await transfersWork(chain, getAddress(token), getAddress((holder as any).wallet ?? "0x0000000000000000000000000000000000000001"), probe);
...
export const allPassed = (gates: GateResult[]) => gates.length > 0 && gates.every((g) => g.passed);
