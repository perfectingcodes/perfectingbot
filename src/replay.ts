...
import { assess, ev, type Evidence } from "./signals/evidence.ts";
import { RiskManager } from "./risk.ts";
import { PaperExecutor } from "./exec/paper.ts";
import { explain } from "./explain.ts";
import { recordSetup } from "./report.ts";
import { normalize } from "./fomo/stream.ts";
import type { Setup } from "./types.ts";

const now = Date.now();

const scenarios: { name: string; token: { address: string; symbol: string }; evidence: Evidence[] }[] = [
  {
...
function run() {
  ...
  const base = { chain: "robinhood" as const, chainId: 4663, side: "buy" as const, token: { address: "0xabc" }, blockTs: now, seenAt: now, source: "onchain" as const };
