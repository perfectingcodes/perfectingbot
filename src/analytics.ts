...
export interface TraderStat {
  wallet: string;
  handle?: string;
  /** Evaluations this wallet appeared in. */
  appearances: number;
  setups: number;
  rejects: number;
  watches: number;
  /** Share of their appearances that cleared every hard check. */
  hitRate: number;
  avgScore: number;
  bestScore: number;
  totalUsd: number;
...
export interface TokenStat {
  ...
  firstSeen: number;
  lastSeen: number;
  /** Score over time — how the read on this token changed as evidence arrived. */
  history: { at: number; score: number; label: string }[];
  buyers: string[];
  failedChecks: string[];
}
 ...
 */
export function traderStats(records: Record_[]): TraderStat[] {
  ...
  for (const r of records) {
    for (const b of r.setup.buyers) {
      ...
      if (!s.handle && b.handle) s.handle = b.handle;
      s.appearances++;
      if (r.assessment.label === "SETUP") s.setups++;
      else if (r.assessment.label === "WATCH") s.watches++;
      else s.rejects++;
      ...
      if (!s.chains.includes(r.setup.chain)) s.chains.push(r.setup.chain);
      const tok = r.setup.token.symbol ?? r.setup.token.address.slice(0, 10);
      if (!s.tokens.includes(tok)) s.tokens.push(tok);
    }
  }
...
export function tokenStats(records: Record_[]): TokenStat[] {
  ...
  for (const r of [...records].sort((a, b) => a.at - b.at)) {
    ...
    for (const b of r.setup.buyers) {
      ...
      if (!t.buyers.includes(who)) t.buyers.push(who);
    }
...
export function gateStats(records: Record_[]) {
  const counts = new Map<string, { name: string; fail: number; pass: number; warn: number; unknown: number; vetoed: number }>();
  for (const r of records) {
    for (const e of r.assessment.evidence) {
      let c = counts.get(e.name);
      if (!c) counts.set(e.name, (c = { name: e.name, fail: 0, pass: 0, warn: 0, unknown: 0, vetoed: 0 }));
      c[e.verdict]++;
      if (e.veto && e.verdict === "fail") c.vetoed++;
    }
  }
  return [...counts.values()].sort((a, b) => b.vetoed - a.vetoed || b.fail - a.fail);
}

export function summary(records: Record_[]) {
