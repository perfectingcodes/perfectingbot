...
import { clientFor } from "./chain/clients.ts";
...
for (const chain of EVM_CHAINS as readonly EvmChain[]) {
  ...
  try {
    ...
    if (id !== config.chains[chain].id) { bad(`${chain}: RPC reports chainId ${id}, expected ${config.chains[chain].id}`); continue; }
    ...
    for (const [label, topic] of [["v2", PAIR_CREATED], ["v3", POOL_CREATED]] as const) {
...
if (!config.fomoKey) {
  ...
  try {
    ...
    if (withEvm === 0) bad("no traders expose an EVM wallet — watchlist would be empty");
    else ok(`${withEvm} traders expose EVM wallets`);
  } catch (e) {
  ...
  try {
    ...
    if (res.ok) ok("token activity endpoint reachable");
...
if (config.mode === "live") bad("MODE=live is not implemented — src/exec/live.ts throws by design");
if (!process.env.SOCIAL_API_URL) warn("SOCIAL_API_URL not set — tier 3 social velocity is unmeasured");
