...
export async function analyze(fomo, trigger, buyers, quality, social, priceUsd) {
    ...
    // Dev behaviour.
    try {
        const devs = await fomo.tokenDevs(trigger.token.address);
        const selling = Array.isArray(devs) ? devs.some((d) => (d?.soldPercent ?? 0) > config.gates.maxDevSoldPercent)
            : (devs?.soldPercent ?? 0) > config.gates.maxDevSoldPercent;
        out.push(ev(1, "dev-selling", selling ? "fail" : "pass", selling ? `dev has sold more than ${config.gates.maxDevSoldPercent}% of their position` : "dev position looks intact", { share: 0.10, veto: true }));
    }
    catch {
    ...
    out.push(ev(2, "consensus", n >= 3 ? "pass" : n >= 2 ? "warn" : "fail", `${n} independent tracked wallet${n === 1 ? "" : "s"} bought within ${Math.round(config.signal.consensusWindowMs / 60_000)}m`, { share: 0.40, credit: Math.min(1, (n - 1) / 4) }));
