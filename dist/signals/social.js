 ...
 */
export class HttpSocialSource {
    name = "http";
    url;
    key;
    mentionsPath;
    qualityPath;
    constructor(url, key = process.env.SOCIAL_API_KEY ?? "", mentionsPath = process.env.SOCIAL_MENTIONS_PATH ?? "mentions", qualityPath = process.env.SOCIAL_QUALITY_PATH ?? "quality_mentions") {
        this.url = url;
        this.key = key;
...
const pluck = (o, path) => path.split(".").reduce((a, k) => (a == null ? a : a[k]), o);
/** Keeps a short per-token series and differentiates it. */
export class SocialTracker {
    series = new Map();
    source;
    keep;
    constructor(source, keep = 30) {
        this.source = source;
        this.keep = keep;
    }
