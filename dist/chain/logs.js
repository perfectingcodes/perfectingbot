...
export async function getLogsChunked(chain, args, opts = {}) {
    ...
    while (cursor >= args.fromBlock && requests < maxRequests) {
        ...
        requests++;
        try {
            const logs = await client.getLogs({ address: args.address, topics: args.topics, fromBlock: from, toBlock: cursor });
...
function matches(log, address, topics) {
    if (address && String(log.address).toLowerCase() !== address.toLowerCase())
        return false;
    if (!topics?.length)
        return true;
    for (let i = 0; i < topics.length; i++) {
        const want = topics[i];
        if (want == null)
            continue; // null = wildcard, same as JSON-RPC semantics
        const got = log.topics?.[i];
        if (!got || String(got).toLowerCase() !== String(want).toLowerCase())
            return false;
    }
    return true;
}
