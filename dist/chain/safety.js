...
import { erc20Abi, ownableAbi, TRANSFER_TOPIC, BURN_TOPIC } from "./abi.js";
 ...
 */
export async function inspectSafety(chain, token, pair) {
    const client = clientFor(chain);
    const notes = [];
    const report = { ownerRenounced: null, mintable: null, lpBurnedOrLockedPct: null, lpRemovedRecently: null, notes };
    // Ownership: renounced means the owner can no longer flip taxes or pause transfers.
    try {
        const owner = await client.readContract({ address: token, abi: ownableAbi, functionName: "owner" }).catch(() => client.readContract({ address: token, abi: ownableAbi, functionName: "getOwner" }));
    ...
    if (pair) {
        ...
        try {
            const [supply, ...sinkBalances] = await Promise.all([
                ...
                ...[...BURN_SINKS].map((sink) => client.readContract({ address: pair, abi: erc20Abi, functionName: "balanceOf", args: [getAddress(sink)] })),
            ]);
...
export async function freshWalletShare(chain, wallets) {
    const client = clientFor(chain);
