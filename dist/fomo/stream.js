...
import { resolveWebSocket } from "../ws-compat.js";
/**
...
export class FomoStream {
    ...
    onTrade(h) { this.handlers.push(h); }
    async start() {
        this.closed = false;
    ...
    connect() {
        ...
        if (!this.WS)
            throw new Error("FomoStream.start() must be awaited before connecting");
        ...
        ws.addEventListener("message", (ev) => {
            ...
            if (msg.type === "welcome") {
                ...
                if (msg.delaySeconds)
                    console.warn(`[stream:${this.opts.chain}] feed is delayed ${msg.delaySeconds}s; the lead-time edge is absent on this plan`);
                return;
            }
            if (msg.type !== "trade" && msg.type !== "alert")
                return;
