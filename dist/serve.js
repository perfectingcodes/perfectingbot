...
function filtered(url) {
    ...
    if (chain && chain !== "all")
        rows = rows.filter((r) => r.setup.chain === chain);
    const source = url.searchParams.get("source");
    // Older records predate the source field; treat those as stream candidates.
    if (source && source !== "all")
        rows = rows.filter((r) => (r.setup.source ?? "stream") === source);
    if (label && label !== "all")
        rows = rows.filter((r) => r.assessment.label === label);
    if (minScore > 0)
        rows = rows.filter((r) => r.assessment.score >= minScore);
    if (sinceHours > 0) {
        const cut = Date.now() - sinceHours * 3_600_000;
        rows = rows.filter((r) => r.at >= cut);
    }
    if (q) {
        rows = rows.filter((r) => (r.setup.token.symbol ?? "").toLowerCase().includes(q) ||
        ...
        try {
            switch (url.pathname) {
                ...
                case "/api/leaderboard": {
                    ...
                    if (!config.fomoKey)
                        return json(res, { available: false, reason: "FOMO_API_KEY not set" });
                    try {
...
export function startDashboard(port = Number(process.env.PORT ?? 5173)) {
    ...
    server.listen(port, "0.0.0.0", () => {
        ...
        if (n === 0)
            console.log(`[serve] no evaluations yet — run \`npm run replay\` to populate it`);
    });
    return server;
}
